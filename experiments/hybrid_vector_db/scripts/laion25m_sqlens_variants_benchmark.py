...
from laion25m_pgvector_filtered_benchmark import parse_ids, parse_int_array, recall_at_k, truth_key
...
def parse_selected(path: Path, targets: list[float], limit_per_group: int) -> list[dict[str, Any]]:
    ...
    with path.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            row["qid"] = int(row["qid"])
            row["labels_list"] = parse_int_array(row.get("labels", ""))
            rows.append(row)
    if not rows:
        raise SystemExit("no selected LAION25M rows after filtering")
    return rows


def load_truth(path: Path) -> dict[str, list[int]]:
    out: dict[str, list[int]] = {}
    with path.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            out[row.get("truth_key") or truth_key(row)] = parse_ids(row.get("gt", ""))
    return out


def tag_atoms(labels: list[int]) -> list[str]:
    out: list[str] = []
    for label in labels:
        if out:
            out.append("|")
        out.append(f"sql:labels @> ARRAY[{int(label)}]::int[]")
    return out


def guidance_atoms(row: dict[str, Any]) -> list[str]:
    labels = list(row.get("labels_list") or [])
    workload = str(row.get("workload", "label_or"))
    if workload in {"label", "label_or"} and labels:
        return tag_atoms(labels)
    if labels:
        return tag_atoms(labels)
    predicate = str(row.get("predicate", "")).strip()
    if not predicate:
        raise ValueError(f"row has neither labels nor predicate: {row}")
    return [f"sql:{predicate}"]


def predicted_skip_rate(row: dict[str, Any]) -> float:
    actual_pct = float(row.get("actual_pct", 100.0) or 100.0)
    return max(0.0, min(1.0, 1.0 - actual_pct / 100.0))


def guidance_can_compose_exact_or(atoms: list[str]) -> bool:
    groups = 1
    atoms_in_group = 0
    saw_or = False
    for atom in atoms:
        normalized = atom.strip()
        if normalized == "|" or normalized.upper() == "OR":
            if atoms_in_group != 1:
                return False
            groups += 1
            atoms_in_group = 0
...
def method_table_index(args: argparse.Namespace, method: str) -> tuple[str, str]:
    ...
    return args.table, args.stock_index
...
def configure(cur: psycopg.Cursor, args: argparse.Namespace, method: str) -> None:
    ...
    try:
        cur.execute(
            f"SET hnsw.guidance_compose_exact_or = {'on' if method == 'd1_d2_d3' and args.guidance_compose_exact_or else 'off'}"
        )
    except Exception:
...
def activate_guidance(cur: psycopg.Cursor, args: argparse.Namespace, method: str, row: dict[str, Any]) -> tuple[dict[str, Any], float]:
    ...
    if not enabled:
        ...
        return {
            ...
            "table": table,
            "index": index,
            "predicted_skip_rate": predicted_skip_rate(row),
        ...
        out.append(
            {
                ...
                "predicted_skip_rate_mean": statistics.fmean(vals("predicted_skip_rate")),
                "guidance_enabled_rate": statistics.fmean(vals("guidance_enabled")),
                "d3_guard_can_compose_exact_or_rate": statistics.fmean(vals("d3_guard_can_compose_exact_or")),
                "d3_guard_disabled_after_activation_rate": statistics.fmean(vals("d3_guard_disabled_after_activation")),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run LAION25M Stock/D1/D1+D2/D1+D2+D3 variants.")
    parser.add_argument("--table", default=TABLE)
    parser.add_argument("--bfs-table", default="laion25m_pgvector_bfs")
    parser.add_argument("--query-table", default=QUERY_TABLE)
    parser.add_argument("--stock-index", default=INDEX)
    parser.add_argument("--bfs-index", default="laion25m_pgvector_bfs_embedding_hnsw")
    ...
    parser.add_argument("--out", type=Path, default=Path("results/hybrid_vector_db/laion25m_sqlens_variants_20260716.csv"))
    ...
    parser.add_argument(
        "--d3-enable-policy",
        default="legacy",
        choices=["legacy", "compose_or_skip"],
        help="For D1+D2+D3, compose_or_skip disables guidance unless composed exact is active or predicted skip is high.",
    )
