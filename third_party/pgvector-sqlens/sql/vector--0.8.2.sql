...
CREATE FUNCTION vector_hnsw_last_scan_profile() RETURNS text
...
CREATE FUNCTION vector_hnsw_page_materialize(regclass, vector, int4, int4) RETURNS TABLE(rank int4, id int8, ctid text)
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL UNSAFE;

CREATE FUNCTION vector_hnsw_page_materialize_profile() RETURNS text
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL SAFE;

CREATE FUNCTION vector_hnsw_metadata_cache_build(regclass, text) RETURNS int8
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL UNSAFE;

CREATE FUNCTION vector_hnsw_metadata_page_cache_build(regclass, text) RETURNS int8
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL UNSAFE;
...
CREATE FUNCTION vector_hnsw_metadata_page_filter_candidates(regclass, vector, int4, text) RETURNS TABLE(rank int4, ctid text)
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL UNSAFE;

CREATE FUNCTION vector_hnsw_metadata_bloom_filter_candidates(regclass, vector, int4, text) RETURNS TABLE(rank int4, ctid text)
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL UNSAFE;

CREATE FUNCTION vector_hnsw_metadata_bloom_filter_candidates_limited(regclass, vector, int4, int4, text) RETURNS TABLE(rank int4, ctid text)
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL UNSAFE;

CREATE FUNCTION vector_hnsw_metadata_filter_profile() RETURNS text
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL SAFE;

CREATE FUNCTION vector_hnsw_metadata_cache_profile() RETURNS text
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL SAFE;

CREATE FUNCTION vector_hnsw_metadata_cache_reset() RETURNS void
	AS 'MODULE_PATHNAME' LANGUAGE C VOLATILE PARALLEL SAFE;
